...
public class AccountDecoration extends RecyclerView.ItemDecoration {
    ...
    public AccountDecoration(Context context,List<AccountBean> list){
...
//                Log.d("zyy",index+"");
                ...
                c.drawText(list.get(index).getMonth()+"-"+list.get(index).getDay()
                        ,x+40,baseline,txtPaint);
...
//
    ...
    public boolean isFirstInGroup(int position){
        AccountBean pre,current;

        if(position != 0){
            pre = list.get(position - 1);
            current = list.get(position);

            if( pre.getDay()!=current.getDay()){
                return true;
            }else {
                return false;
            }
